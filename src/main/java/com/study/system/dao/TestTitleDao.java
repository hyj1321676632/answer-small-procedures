...
public interface TestTitleDao  extends JpaRepository<TestTitle,Long> {
    ...
    @Query(value = " select * from test_title where title_id in (select title_id from (select title_id from user_title where user_id = ?1 and \n" +
            ...
            "  select title_id from user_title where user_id = ?1) and rownum <= ?3 )  ",
    ...
    @Query(value = "select * from test_title where title_id not in (select title_id from user_title where user_id = :userId ) and rownum <= :titleCount ",
    ...
    @Query(value = "select * from test_title where title_id in (select title_id from user_title where user_id =:userId and test_result = 'false') and rownum <= :titleCount ",
    ...
    @Query(value = " select * from test_title where  rownum <= ?1 ",
